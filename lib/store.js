...
var Store = function(options) {
    ...
    if (options.redis) {
        ...
        self.subscriber = redis.createClient(options.redis.port, options.redis.host, options.redis.options);
        redisClientEvents(self.subscriber);

        self.publisher = redis.createClient(options.redis.port, options.redis.host, options.redis.options);
        ...
        // TODO: hack to keep connection alive
        var healthCheck = function() {
            self.subscriber.set('dirty-cache:_tmp_sub', new Date().toISOString(), function(err) {
                if (err) {
                    self.emit('error', err);
                }
            });

            self.publisher.set('dirty-cache:_tmp_pub', new Date().toISOString(), function(err) {
                if (err) {
                    self.emit('error', err);
                }
            });

            setTimeout(healthCheck, Math.floor(Math.random() * (300 + 1)) * 1000);
        };
